...
fish_leslie <- matrix(c(0, 0, 0, 600, 0.05, 0, 0, 0, 0, 0.20, 0, 0, 0, 0, 0.16, 0.9), 
...
# The line below making a vector from 0-8 by 1
time_yr <- seq(from = 0, to = 8, by = 1) 

# The line below making an empty matrix that will store the values from the function. This will have the number of fish_year0 columns (4), and number of time_yr rows (9)
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr)) 

# The line below will populate the first row of the proj_year matrix using fish_year0 values
proj_year[1, ] <- fish_year0

# The for loop below starts at row 2 to populate matrix, and will iterate over the rest of the rows (length of time_yr). For every row, it will populate with the output of the dot product for fish_leslie and the previous year information [i-1]
...
proj_df <- data.frame(time_yr, proj_year) %>% 
  pivot_longer(cols = -time_yr, 
               names_to = "lifestage", 
               values_to = "stage_population")

ggplot(data = proj_df, 
       aes(x = time_yr, y = stage_population)) +
